...
pdf(file = "figures-2026-02-21-nonlinear.pdf", width = 12, height = 12)
...
varnames = c("Density (fish/tank)", "Body Mass (g)", "Summer Temperature (°C)", "Winter Temperature (°C)")
pchvect = c(1,2)
index_treatment = 5
...
for (X_5 in c(0,1)){
  for (X_4 in c(X_min[4], X_mid[4], X_max[4])){
    for (X_3 in c(X_min[3], X_mid[3], X_max[3])){
      ...
      ## Visualise mesh
      breaks = seq(-5,5,1.0)
      image(predictions, xaxt='n', yaxt='n', xlab='Density (fish/tank)', ylab='Body Mass  (g)', col=hcl.colors(length(breaks)-1, "PiYG", rev=FALSE, alpha = 0.75), breaks=breaks)
...
for(k in 1:length(predictions_N)){
  ...
  image(img, col=hcl.colors(length(breaks)-1, "RdYlBu", rev=FALSE, alpha = 0.3), xaxt='n', yaxt='n', xlab='Density (fish/tank)', ylab='Body Mass (g)')
...
## Plot
img = (predictions_N[[k]] > 0) + (predictions_Z[[k]] < 0) * 2
image(img*0, col='white', xaxt='n', yaxt='n', xlab='Density (fish/tank)', ylab='Body Mass (g)')
