...
MTS = MTS[,-which(colnames(MTS)==c("temp_mean_summer"))]
...
N = 4 # number explanatory variables
W_p = 10
N_p = 2 * W_p * (2 + N)
sd1_p = .1
...
colvect = c('blue', 'red', 'cyan', 'magenta')
...
for (selected_TS_ in selected_TS)
{
  ...
  #
  ## Legend
  legend("bottom", legend = c("N:N", "Z:N", "E1:N", "F:N"), col=colvect, horiz=T, bty='n', pch=1)
...
varnames = c("Log Density (S.U.)", "Log Body Length (S.U.)", "Summer Temperature (S.U.)", "Winter Temperature (S.U.)")
pchvect = c(1,2)
...
for (selected_TS_ in selected_TS)
{
  ...
  #
  ## Legend
  legend("bottom", legend = c("N:Z", "Z:Z", "E1:Z", "F:Z"), col=colvect, horiz=T, bty='n', pch=1)
...
par(mfrow=c(N-1,N-1), mar=c(4.5,4.5,1,1), cex.lab=1.5)
colvect = c("lightblue", "salmon")  # Updated to distinguish F_ conditions
varnames = c("Log Density (S.U.)", "Log Body Length (S.U.)", "Summer Temperature (S.U.)", "Winter Temperature")
pchvect = c(1,2)
index_treatment = 4
...
for(k in 1:length(predictions_N)){
  ...
  image(img, col=hcl.colors(length(breaks)-1, "RdYlBu", rev=FALSE, alpha = 0.3), xaxt='n', yaxt='n', xlab='Log Density', ylab='Log Body Size')
  ...
  x1_points_x = (X_[,1] - X_min[1]) / (X_max[1] - X_min[1]) 
  x2_points_x = (X_[,2] - X_min[2]) / (X_max[2] - X_min[2]) 
  points(x1_points_x, x2_points_x, pch=X_[,4]+1, col=c('black','red')[X_[,4]+1])
