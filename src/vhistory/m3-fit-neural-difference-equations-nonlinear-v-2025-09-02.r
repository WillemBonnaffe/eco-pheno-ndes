...
pdf(file = "figures-nonlinear.pdf", width = 12, height = 12)
...
N_p = 2 * W_p * (2 + N)
sd1_p = .1
sd2_p = .05 # N_DJK = .25; Z_DJK = .45; N_BEH = 1.0; Z_BEH = 1.25
rv_idx = 1
...
for (selected_TS_ in selected_TS)
{
  ...
  #
  ## Legend
  legend("bottom", legend = c("∆N(t)"), col=colvect[rv_idx], horiz=T, lty=1, bty='n')
  ...
  #
  ## Legend
  legend("bottom", legend = c("N:N", "Z:N", "T2:N", "T1:N", "F:N"), col=colvect, horiz=T, bty='n', pch=1)
...
colnames(summary_table_effects_on_delta_N) = c("estimate","N:N", "Z:N", "E1:N", "E2:N", "F:N")
write.table(x=summary_table_effects_on_delta_N, file="tab-effects-population.csv", sep=",", row.names=F)
...
for (selected_TS_ in selected_TS)
{
  ...
  #
  ## Legend
  legend("bottom", legend = c("N:N", "Z:N", "E1:N", "E2:N", "F:N"), col=colvect, horiz=T, bty='n', pch=1)
...
for (index_explanatory_variable in c(3, 4))
{
  for (E_ in c(-1.0, 1.5))
  {
    for (s in 1:(N-1))
    {
      ...
      legend("bottom", legend = c("Not Fished", "Fished"), horiz=T, lty=1, col=colvect, bty="n")
    }
  } 
}
par(mfrow=c(1,1))
  ...
  for (E_ in c(0.0))
  {
    for (s in 1:(N-1))
    {
      ...
      for (f_index in 1:2) {
        ...
        y_lo = ddt.N_p$lo
        y_hi = ddt.N_p$hi
        polygon(x=c(x,rev(x)), y=c(y_lo, rev(y_hi)), border=NA, col=adjustcolor(colvect[f_index], 0.1))
        lines(x, y, col=colvect[f_index])
...
for (X_5 in c(0,1)){
  for (X_4 in c(X_min[4], X_mid[4], X_max[4])){
    for (X_3 in c(X_min[3], X_mid[3], X_max[3])){
      ...
      ## Visualise mesh
      breaks = seq(-5,5,1.0)
      image(predictions, xaxt='n', yaxt='n', xlab='Log Density (S.U.)', ylab='Log Body Mass  (S.U.)', col=hcl.colors(length(breaks)-1, "PiYG", rev=FALSE, alpha = 0.75), breaks=breaks)
...
N_p = 2 * W_p * (2 + N)
sd1_p = .1
sd2_p = .35 # N_DJK = .25; Z_DJK = .45; N_BEH = 1.0; Z_BEH = 1.25
rv_idx = 2
...
for (selected_TS_ in selected_TS)
{
  ...
  #
  ## Legend
  legend("bottom", legend = c("∆Z(t)"), col=colvect[rv_idx], horiz=T, lty=1, bty='n')
  ...
  #
  ## Legend
  legend("bottom", legend = c("N:Z", "Z:Z", "E1:Z", "E2:Z", "F:Z"), col=colvect, horiz=T, bty='n', pch=1)
...
colnames(summary_table_effects_on_delta_Z) = c("estimate","N:Z", "Z:Z", "E1:Z", "E2:Z", "F:Z")
write.table(x=summary_table_effects_on_delta_Z, file="tab-effects-phenotype.csv", sep=",", row.names=F)
...
for(k in 1:length(predictions_N)){
  ...
  image(img, col=hcl.colors(length(breaks)-1, "RdYlBu", rev=FALSE, alpha = 0.3), xaxt='n', yaxt='n', xlab='Log Density (S.U.)', ylab='Log Body Mass (S.U.)')
...
## Plot
img = (predictions_N[[k]] > 0) + (predictions_Z[[k]] < 0) * 2
image(img*0, col='white', xaxt='n', yaxt='n', xlab='Log Density (S.U.)', ylab='Log Body Mass (S.U.)')
